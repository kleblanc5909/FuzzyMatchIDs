...
def match2Lists(list1,list2):
    ...
    for member in list1:
        ...
        for i in x:
            TopMatch.append(i[0])
            TopScore.append(i[1])
            TopRowIdx.append(i[2])
    return TopMatch, TopScore, TopRowIdx

def createSecondaryMatch(rowIdxList, headerStr):
    """
...
MatchCat_DRtoC_List = createSecondaryMatch(BestRowIdx_DRtoC, 'MismatchCode')
InvCode_DRtoC_List = createSecondaryMatch(BestRowIdx_DRtoC, 'Inventory_Code')
df['MatchCat_DRtoC']=pd.Series(MatchCat_DRtoC_List)
df['InvCode_DRtoC']= pd.Series(InvCode_DRtoC_List)
...
writer = pd.ExcelWriter('DRMatchedpGUIDsMulti_plusInvCode.xlsx')
